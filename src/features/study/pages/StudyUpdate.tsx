import { AuthHeader } from "@/components/layout/AuthHeader";
import { Badge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/button";
import { Form } from "@/components/ui/Form";
import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import {
  studyCreateSchema,
  type StudyUpdateRequest,
  type TStudySchema,
} from "../validators/study";
import { FormField } from "@/components/ui/FormField";
import StateOnSvg from "@/assets/icons/stateOn.svg";
import StateOffSvg from "@/assets/icons/stateOff.svg";
import { studyUpdateApi } from "../api/study";
import type { ApiResponse } from "@/lib/api/apiClient";
import type { AxiosError } from "axios";
import type { StudyDetail } from "../api/studyType";
import { studyQueryKeys } from "../api/queries";
import { useMutation, useQuery } from "@tanstack/react-query";
import { fetchStudyById } from "../api/study";

// 스터디 분야, 스터디 성향, 진행 방식, 연락 방식 드롭다운 옵션
const STUDY_FIELD_OPTIONS = [
  "학업",
  "언어",
  "취업/커리어",
  "자기계발",
] as const;
const SUBFIELD_MAP: Record<(typeof STUDY_FIELD_OPTIONS)[number], string[]> = {
  학업: ["전공 공부", "시험 공부", "자격증", "고시·임용·공무원"],
  언어: ["회화", "외국어 시험"],
  "취업/커리어": [
    "면접·자소서",
    "디자인",
    "IT",
    "마케팅",
    "코딩",
    "데이터 분석",
  ],
  자기계발: ["독서·글쓰기", "운동", "사진·영상"],
};

const STUDY_TYPE_OPTIONS = ["체계적인", "자유로운", "협력적인", "실적중심"];
const PROGRESS_METHOD_OPTIONS = ["온라인", "오프라인", "온라인/오프라인"];
const CONTACT_METHOD_OPTIONS = ["카카오톡", "이메일"];
const RULE_TAG_OPTIONS = ["시간", "벌금", "휴무", "분위기", "기타"];
const ALLOWED_RULE_CATEGORIES = new Set(RULE_TAG_OPTIONS);

export function StudyUpdate() {
  // const queryClient = useQueryClient();
  const navigate = useNavigate();
  const { studyId } = useParams();
  const studyIdNumber = Number(studyId);
  const [activeRuleTags, setActiveRuleTags] = useState<string[]>([]);
  const [isRecruiting, setIsRecruiting] = useState(false);

  // 스터디 상세 조회
  const { data: studyData } = useQuery<StudyDetail>({
    queryKey: studyQueryKeys.studyDetail(studyIdNumber),
    queryFn: () => fetchStudyById(studyIdNumber),
    enabled: !!studyId,
  });
  const defaultValues = studyData
    ? {
        ...studyData,
        rules: Object.fromEntries(
          (studyData.ruleDTO ?? [])
            .filter((r) => r.description && r.description.trim().length > 0)
            .map((r) => [r.ruleCategory, r.description])
        ),
      }
    : undefined;

  // 스터디 수정 API
  const { mutate: updateStudy } = useMutation<
    StudyDetail,
    AxiosError<ApiResponse<null>>,
    { studyId: number; data: StudyUpdateRequest }
  >({
    mutationFn: ({ studyId, data }) => studyUpdateApi(studyId, data),
    onSuccess: async (_data, vars) => {
      alert("스터디가 수정되었습니다.");
      console.log("Study updated:", _data);
      // queryClient.invalidateQueries({
      //   queryKey: studyQueryKeys.studyDetail(vars.studyId),
      // });
      navigate(`/study/detail/${vars.studyId}`);
    },
    onError: (error) => {
      alert(error.response?.data?.message || "스터디 수정에 실패했습니다.");
    },
  });

  const onSubmit = (data: TStudySchema) => {
    if (!studyId) return;
    const rulesObj = (data as any).rules ?? {};

    const ruleDto = Object.entries(rulesObj)
      .filter(([key]) => activeRuleTags.includes(key))
      .filter(([key]) => ALLOWED_RULE_CATEGORIES.has(key))
      .map(([key, value]) => ({
        ruleCategory: key,
        description: String(value ?? "").trim(),
      }))
      .filter((r) => r.description.length > 0);

    const finalData: StudyUpdateRequest = {
      ...(data as any),
      ruleDTO: ruleDto.length ? ruleDto : undefined,
      isRecruiting: isRecruiting,
    };

    console.log("Study Update Data: ", finalData);
    console.log("isRecruiting:", isRecruiting);

    updateStudy({ studyId: studyIdNumber, data: finalData });
  };

  const toggleRecruiting = () => {
    setIsRecruiting((prev) => !prev);
  };

  useEffect(() => {
    if (!studyData) return;
    setIsRecruiting(studyData.isRecruiting);

    setActiveRuleTags(
      (studyData.ruleDTO ?? [])
        .filter((r) => r.description && r.description.trim().length > 0)
        .map((r) => r.ruleCategory)
    );
  }, [studyData]);

  return (
    <div className="flex min-h-screen w-full flex-col items-center bg-white">
      <AuthHeader />
      <main className="w-full max-w-[800px] px-4 py-10 mt-[88px]">
        <Form
          key={
            studyData ? `loaded-${studyIdNumber}` : `loading-${studyIdNumber}`
          }
          schema={studyCreateSchema}
          onSubmit={onSubmit}
          defaultValues={defaultValues}
          className="space-y-5"
        >
          <div className="flex gap-2 justify-end items-center mb-5">
            <span className="text-body-1-semibold text-gray-400">
              {isRecruiting ? "모집중" : "모집마감"}
            </span>
            <button
              type="button"
              onClick={toggleRecruiting}
              aria-label="모집중/모집마감"
            >
              <img
                src={isRecruiting ? StateOnSvg : StateOffSvg}
                alt=""
                className="w-12 h-12"
              />
            </button>
          </div>

          <div className="w-full flex justify-between items-center mb-6">
            <div className="flex items-center gap-2">
              <Badge variant="blue">1</Badge>
              <h1 className="text-body-1-semibold text-gray-400">
                스터디 기본 정보를 입력해주세요.
              </h1>
            </div>
          </div>
          <FormField
            name="studyFieldName"
            label="스터디 분야"
            placeholder="선택해주세요."
            type="dropdownTwoLevel"
            options={SUBFIELD_MAP}
          />
          <FormField
            name="studyStyleCategory"
            label="스터디 성향"
            placeholder="선택해주세요."
            type="dropdown"
            options={STUDY_TYPE_OPTIONS}
          />
          <FormField
            name="studyFormat"
            label="진행 방식"
            placeholder="선택해주세요."
            type="dropdown"
            options={PROGRESS_METHOD_OPTIONS}
          />
          <div className="flex items-end gap-6">
            <div className="w-1/2">
              <FormField
                name="contactType"
                label="연락 방식"
                placeholder="선택해주세요."
                type="dropdown"
                options={CONTACT_METHOD_OPTIONS}
              />
            </div>
            <div className="w-1/2">
              <FormField name="url" placeholder="연락 정보를 입력해주세요." />
            </div>
          </div>
          <div className="w-full flex justify-between items-center mt-20 mb-6">
            <div className="flex items-center gap-2">
              <Badge variant="blue">2</Badge>
              <h1 className="text-body-1-semibold text-gray-400">
                스터디에 대해 소개해주세요.
              </h1>
            </div>
          </div>
          <FormField
            name="title"
            label="스터디 이름"
            placeholder="스터디 이름을 입력해주세요."
          />
          <FormField
            name="description"
            label="스터디 소개"
            placeholder="스터디를 자유롭게 소개해주세요."
            type="textarea"
          />

          <div className="w-full flex justify-between items-center mt-20 mb-6">
            <div className="flex items-center gap-2">
              <Badge variant="blue">3</Badge>
              <h1 className="text-body-1-semibold text-gray-400">
                스터디 규칙을 정해주세요.
              </h1>
            </div>
          </div>
          <div className="flex gap-2 mb-5">
            {RULE_TAG_OPTIONS.map((rule) => (
              <Button
                key={rule}
                variant={activeRuleTags.includes(rule) ? "focus" : "default"}
                size="sm"
                type="button"
                onClick={() => {
                  setActiveRuleTags((prev) =>
                    prev.includes(rule)
                      ? prev.filter((id) => id !== rule)
                      : [...prev, rule]
                  );
                }}
              >
                {rule}
              </Button>
            ))}
          </div>
          {activeRuleTags.map((key) => {
            const rule = RULE_TAG_OPTIONS.find((rule) => rule === key);
            if (!rule) return null;

            return (
              <FormField
                key={rule}
                name={`rules.${key}`}
                label={`[${rule}] 규칙`}
                placeholder={`${rule}에 대한 규칙을 입력해주세요.`}
                type="textarea"
              />
            );
          })}
          <div className="flex w-full justify-end gap-2 mt-20">
            <Button
              type="button"
              variant="default"
              size="lg"
              onClick={() => navigate(-1)}
            >
              취소
            </Button>
            <Button type="submit" variant="primary" size="lg">
              수정하기
            </Button>
          </div>
        </Form>
      </main>
    </div>
  );
}
